...
class Yuqing_Model extends CI_Model
{
        ...
        switch ($type) {
            case 'trash':
                foreach ($yid_mongo as $one) {
                    $this->mongo->where('_id', $one)->inc(array('yq_trash_num' => 100))->update('rawdata');//标记为100不再显示
                    $this->mongo->where('_id', $one)->set(array('yq_commit_uid' => $uid, 'yq_commit_user' => $this->session->name, 'yq_type' => 'trash'))->update('rawdata');
                }
                $res = true;
                break;
            case 'useless':
                foreach ($yid_mongo as $one) {
                    $this->mongo->where('_id', $one)->inc(array('yq_trash_num' => 100))->update('rawdata');//标记为100不再显示
                    $this->mongo->where('_id', $one)->set(array('yq_commit_uid' => $uid, 'yq_commit_user' => $this->session->name, 'yq_type' => 'useless'))->update('rawdata');
                }
                $res = true;
                break;
            case 'yuqing':
                foreach ($yid_mongo as $one) {
                    $this->mongo->where('_id', $one)->inc(array('yq_trash_num' => 100))->update('rawdata');//标记为100不再显示
                    $this->mongo->where('_id', $one)->set(array('yq_commit_uid' => $uid, 'yq_commit_user' => $this->session->name, 'yq_type' => 'yuqing'))->update('rawdata');
                }
                $res = true;
                break;
            default:
    ...
    public function get_has_rep_yqData($query, $page_num)
    {
        ...
        switch ($query['tag']) {
            case '广告':
                $query_where['yq_type'] = 'trash';
                break;
            case '舆情':
                $query_where['yq_type'] = 'yuqing';
                break;
            case '忽略':
                $query_where['yq_type'] = 'useless';
                break;
        }
        if (!empty($query['search'])) {
            $query_where['$or'] = array(
