...
function add_content_all(data){
    ...
    if(data.info.length !== 0){
        ...
        for(; i<len;i++){
            ...
            str += '<span><a target="_blank" href="/yuqing/rec_detail?'+obj._id.$id+'#record" title="'+obj.title+'">'+obj.title+'</a></span>';
            str += '</td>';
...
function add_content_title(data){
    ...
    if(data.info.length !== 0){
        ...
        for(; i<len;i++){
            ...
            str += '<td colspan="2">';
            str += '<input type="checkbox" data-id="'+obj._id.$id+'">';
            str += '<span><a target="_blank" href="/yuqing/rec_detail?yid='+obj._id.$id+'">'+obj.title+'</a></span>';
            str += '</td>';
            str += '<td>【'+(obj.source?obj.source:'')+'】</td>';
            str += '<td><span class="label label-danger">'+obj.yq_tag+'</span></td>';
            switch (obj.is_cfm){
