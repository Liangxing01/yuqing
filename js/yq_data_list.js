...
function sroll_ajax(type){
    ...
    var arr = {"sort":arr_all[3],"length":page_length,"search":arr_all[4],"media_type":arr_all[1],"yq_tag":arr_all[5]};
    $.ajax({
        type:'POST',
...
function add_content_all(data){
    ...
    if(data.info.length !== 0){
        ...
        for(; i<len;i++){
            ...
            str += '<span><a target="_blank" href="/yuqing/yq_detail?'+obj._id.$id+'" title="'+obj.title+'">'+obj.title+'</a></span>';
            str += '</td>';
...
function add_content_title(data){
    ...
    if(data.info.length !== 0){
        ...
        for(; i<len;i++){
            ...
            str += '<td colspan="2">';
            str += '<input type="checkbox" data-id="'+obj._id.$id+'">';
            str += '<span><a target="_blank" href="/yuqing/yq_detail?'+obj._id.$id+'">'+obj.title+'</a></span>';
            str += '</td>';
            str += '<td>【'+(obj.source?obj.source:'')+'】</td>';
            str += '<td><span class="label label-danger">'+obj.yq_tag+'</span></td>';
            str += '<td>'+timeToDate(obj.yq_pubdate*1000)+'</td>';
